...
- [`references/shopify-lookup.md`](references/shopify-lookup.md) — Dùng MCP tool nào để tra order/customer/product. **Chỉ đọc ở Bước 4 KHI thật sự phải gọi Shopify** (sidebar Zendesk không đủ order/fulfillment/tracking, hoặc cần chi tiết sản phẩm). Nếu sidebar đã đủ → skip Shopify → khỏi cần file này.
- [`references/feedback-loop.md`](references/feedback-loop.md) — Vòng phản hồi & học lại qua Lark Base. Chỉ cần ở **Bước 6** (seed feedback record: base token + field) và **Learning Mode** (Chế độ C, skill riêng `cs-ticket-learn`). Việc fix skill từ feedback diễn ra khi user chủ động gọi (sau khi CS chấm, ~7 ngày), không phải mỗi lần draft. **Quan trọng: KHÔNG tự sửa file skill khi chưa có user duyệt.**
