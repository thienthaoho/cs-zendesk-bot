...
2. **KHÔNG BỊA SỐ LIỆU CỤ THỂ.** Tracking number, ngày ship/giao **cụ thể**, số tiền refund, tên/variant sản phẩm → BẮT BUỘC lấy đúng từ Shopify/Zendesk (khách kiểm tra được, chế là lộ). *Riêng framing trạng thái xử lý/giao hàng* (vd "đã ship out") → theo draft-rules §A: được dùng lý do hợp lý để trấn an, miễn KHÔNG kèm số liệu giả.
...
**2.2 Dữ liệu đơn — ưu tiên Zendesk sidebar; thiếu thì chạy script (KHÔNG gọi MCP tay nếu script chạy được):**
- Sidebar đã đủ order# + fulfillment + tracking → dùng luôn.
- Thiếu, hoặc chạy headless → **chạy script lấy đơn gọn:**
  ```bash
  node ".claude/skills/cs-ticket-process/scripts/shopify-order.mjs" --order <FLWSP...>   # hoặc --email <email>
  ```
  Trả JSON gọn (fulfillment, tracking, line_items, địa chỉ, tuổi đơn, đơn nào active). Script lỗi → fallback MCP `shopify` tay theo [`references/shopify-lookup.md`](references/shopify-lookup.md).

**2.3 Tính điều kiện policy bằng code (đừng nhẩm ngày tay):**
```bash
node ".claude/skills/cs-ticket-process/scripts/policy-dates.mjs" --order-date <ngày> --product-type <flag|non-flag> [--received-date ..] [--last-tracking-update ..] [--carrier usps|international]
```
Trả: business-day bucket, hạn refund còn mấy ngày, cửa sổ sửa 12h, lost-in-transit đủ điều kiện chưa, ETA, dịp lễ gần nhất + có nâng priority không. Đây là **con số/quyết định**.
- Dựa kết quả + đọc **[`references/cs-rules.md`](references/cs-rules.md)** cho phần **diễn giải + ngoại lệ + thẩm quyền duyệt** (refund >50% / cancel sau SX / resend intl → `cs-need-approval`). Phần chữ này code không thay được.

## Bước 3 — Triage + Persona (sau khi đủ facts)

- Phân loại: order/shipping/refund-return/product-question/damaged-wrong/cancellation/address-change/khác. Gán ưu tiên P1–P4 theo SLA trong `cs-rules.md`. **Nếu `policy-dates.mjs` báo `priority_bump: true`** (gần dịp lễ lớn) → tự nâng P1/P2.
- **Xác nhận persona:** nếu có tín hiệu persona (memorial, military, religious, pet, holiday, gift...) → **đọc [`references/personas.md`](references/personas.md)** để chọn đúng persona + tone. Đây là **phán đoán tone, giữ text** — code không thay. Không có tín hiệu rõ → bỏ qua, khỏi đọc file này.
